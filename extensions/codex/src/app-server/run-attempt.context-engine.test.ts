...
function expectRequestInputTextContains(
  ...
  expect(
    input.some((entry) => {
      ...
      return item.type === "text" && text.includes(expected);
    }),
  ).toBe(true);
}
...
describe("runCodexAppServerAttempt context-engine lifecycle", () => {
  ...
  it("bootstraps and assembles non-legacy context before the Codex turn starts", async () => {
    ...
    const developerInstructions =
      typeof threadStartParams.developerInstructions === "string"
        ? threadStartParams.developerInstructions
        : "";
    expect(developerInstructions).toContain("context-engine system");
  ...
  it("calls afterTurn with the mirrored transcript and runs turn maintenance", async () => {
    ...
    const afterTurnCall = requireFirstCallArg(
      afterTurn,
      "afterTurn",
    ) as Parameters<NonNullable<ContextEngine["afterTurn"]>>[0];
