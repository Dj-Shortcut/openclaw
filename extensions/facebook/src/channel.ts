...
const messengerSecurityAdapter = createRestrictSendersChannelSecurity<ResolvedMessengerAccount>({
  ...
  normalizeDmEntry: (raw) => raw.replace(/^messenger:(?:user:)?/i, ""),
});
...
export const messengerPlugin: ChannelPlugin<ResolvedMessengerAccount> = createChatChannelPlugin({
    ...
    messaging: {
      ...
      targetResolver: {
        looksLikeId: (id) => {
          ...
          return Boolean(trimmed && (/^\d{6,}$/.test(trimmed) || /^messenger:/i.test(trimmed)));
        },
        hint: "<page-scoped-id>",
      },
    },
  ...
  pairing: {
    text: {
      ...
      normalizeAllowEntry: createPairingPrefixStripper(/^messenger:(?:user:)?/i),
